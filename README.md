...
| 단계 | 파일 | 설명 |
|------|------|------|
| 데이터 파싱 실험 | `processing_and_upsert.py` | EasyOCR → Clova OCR → pdfplumber bbox 전환 과정 |
...
수집한 전국 공영장례 조례집 PDF는 텍스트가 2~3단 컬럼으로 배치되어 있으나, **표(table) 구조가 없어** 단순 텍스트 추출 시 행·열 구분이 무너지는 문제가 있었습니다.
...
이 로직으로도 정확하게 추출되지 않는 복잡한 페이지(표 형태가 불규칙한 43개 페이지)는 `complex_texts` 딕셔너리에 수동 매핑하여 교체하는 방식으로 보완했습니다.
...
봉안당, 묘지 등의 원본 데이터에는 종교 구분이나 공설/사설 구분 컬럼이 없었습니다. 이를 해결하기 위해:
...
이를 통해 사용자가 "천주교 납골당"이나 "공설 묘지"처럼 질의할 때 의미 기반 검색이 가능해졌습니다.
...
│       ├── empathy_agent.py        # 감성 챗봇 (타 팀원)
│       └── diary_manager.py        # 다이어리 (타 팀원)
├── scripts/
│   ├── insert_db_funeral_facilities.py    # 시설 데이터 적재 (본인 담당)
│   ├── insert_db_funeral_ordinance.py     # 조례 데이터 적재 (본인 담당)
│   └── processing_and_upsert.py           # 파싱 실험 과정 (본인 담당)
├── data/
│   ├── raw/                        # 원본 PDF·CSV
│   ├── processed/                  # 전처리 결과물
